...
$(document).ready(function () {
	...
	$('#calc_one_against_all').click(function () {
		...
		if(ticker1 !== "" && startDate !== "" && lengthOfPeriod !== "" && timeFrame !== "" ){
			...
			$.ajax({
				...
				data: {ticker1: ticker1, startDate: startDate, lengthOfPeriod: lengthOfPeriod, lag: lag_value, timeFrame: timeFrame, calculation_type: calculation_type}
			}).done(function (table_content) {
	...
	$('#insert_new_ticker').click(function () {
		...
		if(ticker !== ""){
			$.ajax({
				...
				data: {ticker: ticker, company: company, exchange: exchange, country: country,
	...
	$('#prediction').click(function() {
		...
		if(ticker_prediction !== "" && days_back !== "" && ticker_prediction_n !== ""){
			...
			$.ajax({
				...
				data: {ticker_prediction: ticker_prediction, days_back: days_back, ticker_prediction_n: ticker_prediction_n}
			}).done(function (table_content) {
	...
	$('.iphone-toggle').on('change', function() {
		...
		$.ajax({
			...
			data: {toggle: toggle_data, value: toggle_value, ticker1: ticker1, ticker2: ticker2, date: date, delay: delay,
